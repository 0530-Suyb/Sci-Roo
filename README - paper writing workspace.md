...
- Ready：已经验证通过。通常是 DOI 或标题匹配结果足够可靠。
- Unverified：还没有被可靠验证，但也不是明确有问题。包括还没查、网络失败、元数据不够、结果过期需要重查这些情况。
- Flagged：查过了，而且发现有明显风险。比如标题冲突、低置信匹配、查不到来源这类，值得优先人工处理。
- Missing：文稿里用了这个 cite key，但本地 reference library 里根本没有对应条目。
- Placeholder：这不是引用校验状态，而是文稿里还有 [CITATION NEEDED] 这种“这里还没决定引用谁”的占位。
