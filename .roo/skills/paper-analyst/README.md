...
Sci-Roo 将本 skill vendored 到项目内的 `.roo/skills/paper-analyst/`。不要依赖 `~/.claude/skills`、`~/.codex/skills` 或其他机器本地目录；项目迁移时随仓库一起携带本目录即可。

### 使用

在 ReadPaper 的 PDF 分析入口中选择 PDF 后，会打开 `sci-lit-review` Chat，并预填使用 `.roo/skills/paper-analyst` 的分析 prompt。也可以在 Chat 中直接用自然语言触发：

```
使用项目本地 skill `.roo/skills/paper-analyst` 分析 reference/paper.pdf
quick summary of this paper
分析这篇文献与前作的关系
将以上分析内容做成一个可以直接用来汇报的组会 PPT，要求美观带图片
```
