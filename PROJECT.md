...
| 目录                          | 作用                                                                             |
| ----------------------------- | -------------------------------------------------------------------------------- |
| `src/core/tools/`             | Agent 工具（ApplyDiff、EditFile、ReadFile、ExecuteCommand、SearchLiterature...） |
...
| `src/core/config/`            | 配置管理（ContextProxy、ProviderSettingsManager）                                |
| `src/core/assistant-message/` | 助手消息渲染（工具调用描述、用户确认 UI）                                        |
| `src/core/checkpoints/`       | 任务检查点/回滚                                                                  |
| `src/services/literature/`    | 文献管理器（LiteratureManager — Sci-Roo 新增）                                   |
...
| `src/shared/`                 | 共享工具（ExtensionState、WebviewMessage、ProfileValidator）                     |
...
| `src/components/literature/`  | 文献库面板（LiteratureView — Sci-Roo 新增）                      |
...
`App.tsx` 中定义 `Tab` 类型：`"chat" | "settings" | "history" | "marketplace" | "cloud" | "literature"`
...
用户点击按钮 → registerCommands.ts 触发命令
  → ClineProvider.postMessageToWebview() 发送消息到前端
    → App.tsx onMessage() 接收并切换 Tab
      → 渲染对应组件（ChatView / LiteratureView / SettingsView...）

前端用户操作 → vscode.postMessage() 发送消息到后端
  → webviewMessageHandler.ts 分发处理
    → 调用对应服务（LiteratureManager / McpHub...）
```
...
| `src/core/webview/literatureMessageHandler.ts`     | 文献相关消息处理（Sci-Roo 新增） |
...
| 路径                | 作用                                        |
| ------------------- | ------------------------------------------- |
| `.roomodes`         | 自定义模式定义（Sci-Roo 6 个科研模式）      |
| `.roo/rules/`       | 全局规则（科研伦理、科学严谨性、引用标准）  |
| `.roo/rules-sci-*/` | 各科研模式专用规则                          |
| `.roo/skills/`      | 技能定义（文献检索、统计检验、图表生成...） |
